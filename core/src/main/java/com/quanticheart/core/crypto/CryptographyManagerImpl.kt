...
import com.quanticheart.core.prefs
...
internal class CryptographyManagerImpl : CryptographyManager {
    ...
    override fun persistBiometricToSharedPrefs(
        context: Context,
        keyName: String,
        ciphertextWrapper: CiphertextWrapper
    ) {
        val json = Gson().toJson(ciphertextWrapper)
        context.prefs.edit().putString(keyName, json).apply()
    }

    override fun getBiometricFromSharedPrefs(
        context: Context,
        keyName: String
    ): CiphertextWrapper? {
        val json = context.prefs.getString(keyName, null)
        return Gson().fromJson(json, CiphertextWrapper::class.java)
    }

    override fun clearBiometricFromSharedPrefs(
        context: Context,
        keyName: String
    ): Boolean {
        return try {
            context.prefs.edit().remove(keyName).apply()
            true
        } catch (e: Exception) {
            false
        }
